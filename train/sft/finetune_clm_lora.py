...
import transformers
from transformers.trainer_utils import PREFIX_CHECKPOINT_DIR, speed_metrics, PredictionOutput
...
class DataTrainingArguments:
    ...
    data_source: str = field(default="WA", metadata={"help": "WA or IL dataset"})
...
meta_data = {
    ...
    "IL":{
        ...
        "special_tokens":{
            ...
            "severity": ["<NO APPARENT INJURY>", "<POSSIBLE INJURY>", "<MINOR INJURY>", "<SERIOUS INJURY>", "<FATAL>"],
            "accident_type": [
                "<FRONT TO REAR>", "<TURNING>", "<SIDESWIPE SAME DIRECTION>", "<PARKED MOTO VEHICLE>", "<COLLISION WITH OBJECTS>", "<ANGLE IMPACTS>",
                "<ANIMAL COLLISIONS>", "<PEDESTRIAN COLLISIONS>", "<SIDESWIPE OPPOSITE DIRECTION>", "<OVERTURNED>", "<PEDALCYCLIST COLLISIONS>", "<FRONT TO FRONT>",
                "<REAR TO FRONT>", "<OTHERS>", "<REAR TO SIDE>", "<REAR TO REAR>",
            ]
        },
        "tokens_explanation":{
            "number_of_injuried_people": "[<ZERO>: zero, <ONE>: one, <TWO>: two, <THREE OR MORE THAN THREE>: three or more than three]",
            "severity": "[<NO APPARENT INJURY>: no apparent injury, <POSSIBLE INJURY>: possible injury, <MINOR INJURY>: minor injury,  <SERIOUS INJURY>: serious injury, <FATAL>: fatal]",
            "accident_type": "[<FRONT TO REAR>: front to rear, <TURNING>: turning, <SIDESWIPE SAME DIRECTION>: sideswipe same direction, <PARKED MOTO VEHICLE>: parked moto vehicle, \
            <COLLISION WITH OBJECTS>: collision with objects, <ANGLE IMPACTS>: angle impacts, <ANIMAL COLLISIONS>: animal collisions, <PEDESTRIAN COLLISIONS>: pedestrian collisions, \
                <SIDESWIPE OPPOSITE DIRECTION>: sideswipe opposite direction, <OVERTURNED>: overturned, <PEDALCYCLIST COLLISIONS>: pedalcyclist collisions, \
                    <FRONT TO FRONT>: front to front, <REAR TO FRONT>: rear to front, <OTHERS>: others, <REAR TO SIDE>: rear to side, <REAR TO REAR>: rear to rear]"
        }    
    }
}
...
def main():
    ...
    only_test = training_args.do_predict and not training_args.do_train
    ...
    if True:
        ...
        logger.info(data_args.validation_files)
        extension = (
        ...
        assert task_type in meta_data[data_source]['task_type']
        description = meta_data[data_source]['description'][task_type]
        special_tokens = meta_data[data_source]['special_tokens'][task_type]
        tokens_explanation = meta_data[data_source]['tokens_explanation'][task_type]

        if 'Llama-3' in model_args.model_name_or_path:
            system_prompt = f'<|begin_of_text|> <|start_header_id|>System<|end_header_id|> You are a helpful assistant to predict the traffic accident.' + \
                f'Please predict the {description} of the accident choosing from the following tokens: {tokens_explanation}.<|eot_id|> \n <|start_header_id|>User<|end_header_id|>'
            gpt_prompt = '<|eot_id|> <|start_header_id|>Assistant<|end_header_id|> The answer is: '
        else:
        ...
        logger.info(f'system prompt: {system_prompt}, gpt_prompt: {gpt_prompt}')
        ...
        raw_datasets["test"] = raw_datasets["test"].map(lambda x:{
            'input': system_prompt + x['text'].split('Assistant:')[0].split('Human:')[1] + gpt_prompt,
            'target': x['text'].split('Assistant:')[1][:-4].strip()
        }, remove_columns=['text'])
    ...
    if len(column_names)==1:
    ...
    elif len(column_names)==2 or ('index' in column_names and len(column_names)==3) or ('ID' in column_names and len(column_names)==3):
        ...
        raise ValueError('Wrong colums in the files')
    ...
    def compute_metrics(eval_preds):
        ...
        return {
            ...
            'precision': metric_pre.compute(predictions=preds, references=labels, average='weighted')['precision'],
    ...
    if training_args.do_eval:
        ...
        if data_args.max_eval_samples is not None:
            ...
            eval_idx = random.sample(range(0, len(eval_dataset)), max_eval_samples)
    ...
    if training_args.do_predict:
        ...
        # save predictions
        labels = output.label_ids[:, 1:]
        preds = output.predictions[:, :-1]
        logger.info(f"logits: {preds[0]}")
        true_predictions = [
                [tokenizer.decode(p) for (p, l) in zip(pred, gold_label) if l != -100]
                for pred, gold_label in zip(preds, labels)
            ]
        true_labels = [
                [tokenizer.decode(l) for (p, l) in zip(pred, gold_label) if l != -100]
                for pred, gold_label in zip(preds, labels)
            ]

        # save json to args.output_dir
        with open(os.path.join(training_args.output_dir, "predictions.json"), "w") as f:
            json.dump({"preds": true_predictions, "labels": true_labels, 
                       'prob': torch.softmax(preds[:, special_token_ids], dim=-1).numpy().tolist()}, f, indent=4)
